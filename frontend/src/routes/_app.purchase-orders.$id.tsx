...
import type { PurchaseOrderDetail } from "@/types/api";
...
export const Route = createFileRoute("/_app/purchase-orders/$id")({
  head: () => ({
    meta: [
      { title: "Purchase order — Urban Furniture Accounting" },
      {
        name: "description",
        content: "Purchase order detail in the Urban Furniture Accounting System.",
      },
      { property: "og:title", content: "Purchase order — Urban Furniture Accounting" },
      {
        property: "og:description",
        content: "Purchase order in the Urban Furniture Accounting System.",
      },
    ],
  }),
  component: () => (
...
function Page() {
  ...
  const confirmMutation = useMutation({
    mutationFn: () => purchaseOrdersService.confirm(id),
    onSuccess: () => {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        crumbs={[{ label: "Purchase" }, { label: "Purchase Orders", to: "/purchase-orders" }]}
        description={po?.vendor?.name}
        actions={
          <>
            <Button variant="outline" onClick={() => navigate({ to: "/purchase-orders/new" })}>
      ...
      {query.isLoading ? <LoadingState label="Loading purchase order" /> : null}
      {query.isError ? <ErrorState error={query.error} onRetry={() => query.refetch()} /> : null}

      {po ? (
        <>
          <PurchaseOrderWorkflowStrip po={po} />
              ...
              <div className="rounded-lg border bg-card p-5 shadow-sm">
                <div className="flex flex-wrap items-start justify-between gap-4">
                  <div>
                    ...
                    <h2 className="mt-1 text-base font-bold text-foreground">{po.po_number}</h2>
                ...
                <dl className="mt-5 grid grid-cols-2 gap-4 text-sm sm:grid-cols-3">
                  <div>
                    ...
                    <dd className="mt-1 font-medium text-foreground">{date(po.order_date)}</dd>
                  </div>
                  <div>
                    <dt className="text-[11px] font-semibold uppercase tracking-[0.06em] text-muted-foreground">
                      Vendor
                    </dt>
                    <dd className="mt-1 font-medium text-foreground">{po.vendor_name}</dd>
                  </div>
                </dl>
              </div>
...
export function PurchaseOrderWorkflowStrip({ po }: { po: PurchaseOrderDetail }) {
  const steps = [
    { label: "Purchase order", done: true },
    { label: "Confirmed", done: po.status !== "draft" },
    { label: "Bill", done: po.status === "confirmed" && !!po.lines.length },
  ];
  return (
