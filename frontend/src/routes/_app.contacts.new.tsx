...
export const Route = createFileRoute("/_app/contacts/new")({
  head: () => ({
    meta: [
      ...
      { property: "og:description", content: "New contact in the Urban Furniture Accounting System." },
    ],
  }),
  component: () => (
...
function Page() {
  ...
  const mutation = useMutation({
    ...
    onSuccess: (contact) => {
      ...
      contact?.id ? navigate({ to: "/contacts/$id", params: { id: contact.id } }) : navigate({ to: "/contacts" });
    },
    onError: (error) => {
